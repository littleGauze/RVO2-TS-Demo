 ...
 */
export class Agent {
    public agentNeighbors: Array<{ key: number; value: Agent }> = [];
    public obstacleNeighbors: Array<{ key: number; value: Obstacle }> = [];
    public orcaLines: Line[] = [];
    public position: Vector2 = new Vector2(0, 0);
    public prefVelocity: Vector2 = new Vector2(0, 0);
    ...
    private newVelocity: Vector2 = new Vector2(0, 0);
     ...
     */
    private linearProgram1(lines: Line[], lineNo: number, radius: number, optVelocity: Vector2, directionOpt: boolean, result: { value: Vector2 }): boolean {
     ...
     * @param radius The radius of the circular constraint.
     */
    private linearProgram3(lines: Line[], numObstLines: number, beginLine: number, radius: number): void {
        let distance = 0.0;
        ...
        for (let i = beginLine; i < lines.length; ++i) {
            if (RVOMath.det(lines[i].direction, lines[i].point.subtract(this.newVelocity)) > distance) {
                ...
                line.point = lines[i].point.add(
                    lines[i].direction.multiply(
                        RVOMath.det(lines[j].direction, lines[i].point.subtract(lines[j].point)) / determinant
                    )
                );
            }
     ...
     */
    public insertAgentNeighbor(agent: Agent, rangeSq: { value: number }): void {
        if (this !== agent) {
